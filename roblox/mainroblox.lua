...
function LuaGB:draw_game_screen()
	...
	if not lastdraw then lastdraw = pixels; print("lastdraw didnt exist") end
	if not lastraw then lastraw = raw_image_data; print("lastraw didnt exist") end
	...
	for y = 0, 143 do
		...
		end; if y % 100 == 0 then task.wait() end
	end
...
while love and script do
	love.update()
	love.draw()
	if shouldwait % 100 == 0 then task.wait() end
end
